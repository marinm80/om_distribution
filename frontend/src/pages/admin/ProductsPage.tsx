...
const ProductsPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = error.response?.data?.message || error.message || 'Error saving product';
      alert(`Error: ${msg}`); 
    }
  };
  ...
  const downloadTemplate = async () => {
    ...
    downloadFile(new Uint8Array(wbout as ArrayBuffer), 'OM_Products_Import_Template.xlsx', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
  };
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      alert(error.response?.data?.message || 'Import error. Check file format.'); 
    }
